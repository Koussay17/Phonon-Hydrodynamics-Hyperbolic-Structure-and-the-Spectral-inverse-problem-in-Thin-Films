...
\toprule
\textbf{Loi} & \textbf{Entropie convexe} & \textbf{Second principe} & \textbf{Propagation} \\
\midrule
Fourier & oui & oui, si $\lambda > 0$ & infinie \\
\addlinespace
Cattaneo & oui, si $\tau_R > 0$ & oui, si $\lambda > 0$ & finie, $\sqrt{a/\tau_R}$ \\
\addlinespace
Guyer--Krumhansl & oui, si $\tau_R > 0$ & oui, si $\lambda > 0$ et $\ell^2 \ge 0$ & infinie \\
\bottomrule
\end{tabularx}
\end{center}

\medskip
Les trois lois sont thermodynamiquement admissibles. \textbf{Une seule propage à vitesse finie.} Le
...
où $\tau_\ell = 3\ell^2/a$ est le temps de diffusion sur la longueur non locale. Cattaneo correspond
...
Deux valeurs propres réelles distinctes : le système est strictement hyperbolique. Le théorème de
Godunov et Mock relie cette propriété à l'existence d'une entropie convexe, ce que la section
suivante établit indépendamment.
...
La partie hors équilibre est une forme quadratique définie négative dès que $\tau_R > 0$ et
$\lambda > 0$ : l'entropie est concave, donc l'état d'équilibre est stable.

\subsection{Productions}

Avec le flux d'entropie $\mathbf{J}_s = \mathbf{q}/T$, la dérivation depuis les équations de bilan
donne, au premier ordre autour d'une température de référence $T_0$,
...
\begin{itemize}
\item il \textbf{ajoute} un paramètre identifiable, $\tau_\ell$, dont l'estimation ne dégrade pas
celle des autres ;
\item il \textbf{ne lève pas} la dégénérescence d'échelle du problème inverse, n'entrant dans
l'observable que par un temps ;
\item il \textbf{détruit} la finitude de la vitesse de propagation.
\end{itemize}

Un gain, une neutralité et une perte, tous trois attribuables au même terme. Le choix entre Cattaneo
...
\textbf{139}, 521 (1961).
\item D. J. Mock, \emph{Systems of conservation laws of mixed type}, J. Differential Equations
\textbf{37}, 70 (1980).
\end{enumerate}}

\vspace{4pt}
\noindent\small
Vérifications : \texttt{tests/test\_admissibility.py}, dont la dérivation symbolique des deux
productions d'entropie. Implantation : \texttt{src/admissibility.py}. Conventions :
\texttt{notes/conventions.md}, section~10. Références à vérifier à la source avant citation.
