...
La dernière inégalité découle de Cauchy--Schwarz ; l'égalité exige une durée commune aux modes de poids non nul. La mémoire ajuste une pente, pas toute la réponse. Aucun des deux temps n'est automatiquement un temps GK.
...
Pour ${\cal C}_R$, conserver seulement $H_R=e$. On a $H_N^T{\cal C}_Ny=0$ et $e^T{\cal C}_Ry=0$. Avec $U=D^{1/2}H$, l'opérateur devient $D^{1/2}[I-U(U^TU)^+U^T]D^{1/2}$ : symétrique positif semi-défini. Le code applique cette projection sans matrice dense. C'est un modèle de relaxation, pas la matrice ab initio des collisions : les diagonales ne fixent pas les couplages.
...
\begin{center}\includegraphics[width=\textwidth]{08_aln_temperature.png}\end{center}
...
\begin{center}\includegraphics[width=0.95\textwidth]{07_aln_spectral.png}\end{center}
