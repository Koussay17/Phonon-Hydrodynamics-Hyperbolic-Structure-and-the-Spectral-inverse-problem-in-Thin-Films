...
Journal of Applied Physics \textbf{137}, 155103 (2025). DOI 10.1063/\allowbreak 5.0257299.
...
(voir §~11). Fiche établie le 12 septembre 2026, corrigée le 16 septembre 2026.
...
\subsection{Le centre de transition \texorpdfstring{$\omega\tau = 1$}{omega tau = 1}}
...
\subsection{Le cas stratifié et l'enchevêtrement par \texorpdfstring{$A_0$}{A0}}
...
\subsection{Contrôle numérique de \texorpdfstring{$S_\tau$}{S tau}}
...
\begin{encadre}
\[
\tau_{\mathrm{centre}} = \frac{1}{2\pi f_{\max}} = 8{,}0\times10^{-10}\ \text{s}
\]
\end{encadre}
Cette valeur localise le centre de transition, pas un temps minimal détectable.
L'indiscernabilité visuelle dans une figure ne remplace pas une analyse de bruit.
À 200~MHz, $\tau=10$~ps produit un écart de phase de $0{,}360^\circ$ dans le
demi-espace idéal. Avec $0{,}01^\circ$ de bruit et tous les autres paramètres connus,
l'incertitude relative locale est $2{,}78\,\%$. Ce contre-exemple réfute une
impossibilité fondée sur la seule bande, sans établir la faisabilité sur un film réel.
...
concordance vérifie la constante de propagation. Le coefficient de flux exige
en plus $\lambda_{\mathrm{eff}}=\lambda/(1+\tau p)$ ; remplacer $p$ dans tout
le quadripôle de Fourier donnerait une phase erronée.
...
présent travail. Dans le demi-espace, $\phi=\arg(E)/2$ : il s'agit de la même
information algébrique, pas d'une validation indépendante ni d'un seuil de détection.
...
\item Article de modélisation directe, sans données ni ajustement. La substitution
      ...
      travail pour la constante de propagation ; le flux requiert la conductivité effective.
...
\item Les auteurs reconnaissent dans le corps du texte que la bande requise dépasse les
      capacités discutées pour leurs configurations. Le centre de transition accessible, à
      200~MHz, est $\tau_{\mathrm{centre}} = 1/(2\pi f_{\max}) = 8{,}0\times10^{-10}$~s.
