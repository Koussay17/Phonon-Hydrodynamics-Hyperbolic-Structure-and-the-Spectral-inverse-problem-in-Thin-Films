...
the reverse reaction already included. A distinct wave-vector-reversed
event is counted separately; a self-reciprocal unordered event is counted once. Prefactors include all counting/volume conventions supplied by the
...
class DecayEventOperator:
    ...
    deliberately outside this contract. For an accepted NONZERO mismatch,
    R.T R is a PSD approximation, not the exact nonlinear Bose Jacobian.
    No response-error bound follows from the resonance tolerance. At large
    x, entropy_scale may underflow to zero: the finite limiting action does
    not certify an invertible population-to-entropy coordinate map.
    """
    ...
    def __init__(self, energy_over_kbt, events, prefactors, resonance_rtol=1e-12):
        ...
        # Compensated summation is essential: (1e16 - 1) - 1e16 falsely
        # reports zero, while the exact mismatch of stored floats is -1.
        try:
            detuning = np.array([math.fsum((float(a), -float(b), -float(c)))
                                 for a, b, c in energies])
        except OverflowError as exc:
            raise ValueError("event energy mismatch exceeds floating-point range") from exc
        residual = detuning / scale
    ...
    def as_linear_operator(self):
        ...
        def matmat(y):
            y = np.asarray(y)
            if y.ndim != 2 or y.shape[0] != n or np.any(~np.isfinite(y)):
                raise ValueError("finite mode matrix with the operator shape required")
            return self.factor.T @ (self.factor @ y)
