...
def test_energy_normal_momentum_psd_and_reversal():
    ...
    np.testing.assert_allclose(c[np.ix_(reversal, reversal)], c, rtol=0, atol=0)
...
@pytest.mark.parametrize("x", [[1e16, 1., 1e16], [2.**53+2, 1., 2.**53]])
@pytest.mark.parametrize("event", [[0, 1, 2], [0, 2, 1]])
def test_compensated_resonance_rejects_hidden_thermal_detuning(x, event):
    from fractions import Fraction
    exact = Fraction(x[0])-Fraction(x[1])-Fraction(x[2])
    assert abs(exact) == 1
    with pytest.raises(ValueError, match="conserve energy"):
        DecayEventOperator(x, [event], [1.])


def test_zero_tolerance_accepts_exact_stored_resonance():
    op = DecayEventOperator([3., 1., 2.], [[0, 1, 2]], [1.], resonance_rtol=0)
    np.testing.assert_array_equal(op.relative_energy_residual, [0.])
    np.testing.assert_array_equal(op.detailed_balance_log_residual, [0.])


def test_nonzero_roundoff_detuning_is_exposed_not_declared_exact():
    from fractions import Fraction
    x = [3.+1e-13, 1., 2.]
    exact = Fraction(x[0])-Fraction(x[1])-Fraction(x[2])
    op = DecayEventOperator(x, [[0, 1, 2]], [1.])
    assert op.detailed_balance_log_residual[0] == float(exact) != 0


def test_nonfinite_multiple_rhs_rejected_consistently():
    op = pair().as_linear_operator()
    for y in (np.full((6, 2), np.nan), np.full((6, 2), np.inf)):
        with pytest.raises(ValueError, match="finite"):
            op@y


def test_extreme_limit_does_not_certify_invertible_entropy_coordinates():
    op = DecayEventOperator([3000., 1000., 2000.], [[0, 1, 2]], [1.])
    assert op.entropy_scale[0] == 0
    assert np.isfinite(dense(op)).all()
