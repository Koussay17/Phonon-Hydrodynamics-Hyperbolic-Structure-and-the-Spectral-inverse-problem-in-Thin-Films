...
import json,os,hashlib,argparse,importlib.metadata
...
BASE=args.pilot_root.resolve()
ROOT=args.output_dir.resolve()
ROOT.mkdir(parents=True,exist_ok=False)
for package in ("phono3py","phonopy"):
 assert importlib.metadata.version(package)=="4.5.0", "Pinned internal API required"
...
block_u=[]
group_record=[]
for v,fa,fb in zip(u,arrays[1.5][0],arrays[2.][0]):
 groups=frequency_blocks(fa)
 other=frequency_blocks(fb)
 if [g.tolist() for g in groups]!=[g.tolist() for g in other]:
  raise ValueError("exports disagree on numerical block partition")
 grouped=np.zeros_like(v)
 for group in groups:
  left,sv,right=np.linalg.svd(v[np.ix_(group,group)])
  if np.min(sv)<1-1e-8:
   raise ValueError("matched eigenspaces are insufficiently aligned")
  grouped[np.ix_(group,group)]=left@right
 if np.linalg.norm(grouped.conj().T@grouped-np.eye(12))>1e-10:
  raise ValueError("block transformation is not unitary")
 block_u.append(grouped)
 group_record.append([g.tolist() for g in groups])
...
result=dict(scope="one selected AlN triplet, shared real-to-reciprocal source, independent complex einsum contraction; not collision action",
 frequency_block_tolerance_THz=1e-10,frequency_blocks=group_record,
...
np.savez_compressed(ROOT/"complex-triplet.npz",amplitude_a=amplitudes[1.5],amplitude_b=amplitudes[2.],predicted_b=predicted_amp,
...
result["block_helper_sha256"]=hashlib.sha256((Path(__file__).parent/"frequency_blocks.py").read_bytes()).hexdigest()
result["script_sha256"]=hashlib.sha256(Path(__file__).read_bytes()).hexdigest()
(ROOT/"complex-triplet.json").write_text(json.dumps(result,indent=2)+"\n")
